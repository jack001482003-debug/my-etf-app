...
沒問題！我已經將「每月定期定額投入」的功能整合進去。現在系統會同時計算：「現有持股的複利」+「每個月新投入資金的滾存」，這能更真實地模擬你長期的資產增長。

這次更新的重點：

新增「每月投入金額」：在側邊欄最上方可以設定每個月預計再拿多少錢買股票。

自動計算加碼張數：系統會根據你設定的「未來資金分配比」，自動把每月投入的錢換算成張數加入複利計算。

FinMind 即時更新：依然保持每次開啟網頁即抓取最新股價。

市值精度修正：保持 (張數 * 價格 * 1000) / 10000。

請將 etf_app.py 內容全部替換：
