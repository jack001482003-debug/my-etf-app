...
st.write(f"### 🎯 目標進度 (月領 {target_monthly} 元)")
st.progress(min(total_current_monthly_income/target_monthly, 1.0))

# 配息日曆
st.write("### 📅 現有持股領息日曆 (預估)")
cal_df = pd.DataFrame({"月份": [f"{m}月" for m in range(1, 13)], "預估金額": [int(calendar[m]) for m in range(1, 13)]})
...
st.info(f"💡 計算說明：補足金額是以平均年化 6% 殖利率估算。如果你選擇的是更高息的標的（如 00918, 00919），所需再投入的本金會比預估的更少。")
